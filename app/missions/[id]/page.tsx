...
export default function MissionDetailPage() {
  ...
  const canReview = user && mission && mission.status === "completed" && user.id === mission.client;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          {isOwner && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 transition-all disabled:opacity-60 shrink-0"
            >
      ...
      {/* ── POSTULER (freelance uniquement, mission ouverte) ── */}
      ...
      {/* ── PAIEMENT (owner, mission en cours) ── */}
      ...
      {/* ── REVIEW (client, mission terminée) ── */}
      {canReview && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="space-y-4">
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((r) => (
                  <button
                    key={r}
                    onClick={() => setRating(r)}
                    className={`text-2xl transition-transform hover:scale-110 ${r <= rating ? "text-amber-400" : "text-gray-200"}`}
                  >
                    ★
                  </button>
                ))}
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-violet-500 to-indigo-600 hover:from-violet-600 hover:to-indigo-700 shadow-md shadow-violet-100 transition-all disabled:opacity-60"
            >
